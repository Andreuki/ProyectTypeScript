...
import type { Property } from "./classes/types";
...
function createAndAppendCard(propertyData :Property): void {
  ...
  const cardClone = (cardTemplate.content.cloneNode(true) as DocumentFragment).firstElementChild as HTMLElement;
  ...
  (cardClone.querySelector(".property-title") as HTMLElement).append(propertyData.title);
  (cardClone
    .querySelector(".property-location")as HTMLElement)
    .append(`${propertyData.address}, ${propertyData.town.name}, ${propertyData.town.province.name}`);
  (cardClone.querySelector(".property-price")as HTMLElement).append(formattedPrice);
  (cardClone
    .querySelector(".property-description")as HTMLElement)
    .append(propertyData.description);
  (cardClone
    .querySelector(".property-sqmeters")as HTMLElement)
    .append(`${propertyData.sqmeters} sqm`);
  (cardClone
    .querySelector(".property-rooms")as HTMLElement)
    .append(`${propertyData.numRooms} beds`);
  (cardClone
    .querySelector(".property-baths")as HTMLElement)
    .append(`${propertyData.numBaths} baths`);
  (cardClone.querySelector(".property-image")as HTMLImageElement).src = propertyData.mainPhoto;

  (cardClone.querySelector(".btn-delete")as HTMLElement).addEventListener('click', async () => {
